    ...
    if transactionChannel_toggle == True:
        if len(transactionChannel) > 1:
            ...
            if find_channel:
                ...
                startup.warn(
                    "Was unable to find " + str(find_channel.id) + " on the given server! Please check your config.")
                await client.close()
...
@client.command(name='getid', aliases=['gid', 'id'], brief='Search tagged objects',
                description='Returns the ID of a member, role, or channel.',
                usage='[@tag_player]/[@tag_team]/[@tag_channel]')
...
@client.command(name='promote', aliases=['p'], brief='Promote user',
                description='Gives tagged player the tagged team_staff rank.',
                usage='[@tag_player] [@tag_team] [@tag_role]')
async def promote(ctx, *args):
    ...
    if ((ctx.message.role_mentions[0] in F_team_staff) and not (ctx.message.role_mentions[1] in F_team_staff)):
        ...
        await ctx.send(
            ":negative_squared_cross_mark: Neither of the roles or both of the roles you mentioned are team staff roles!")
        return
...
@client.command(name='demote', aliases=['d'], brief='Demote user',
                description='Removes all team_staff ranks from a tagged player.',
                usage='[@tag_player] [@tag_team] [@tag_role]')
async def demote(ctx, *args):
    ...
    await ctx.send(
        ":arrow_heading_down: " + ctx.message.mentions[0].mention + " was demoted from " + staff_role.mention)
...
async def sign(ctx, *args):
    ...
    await ctx.send(
        ":pen_fountain: " + ctx.message.mentions[0].mention + " was signed to " + ctx.message.role_mentions[0].mention)
...
async def release(ctx, *args):
    ...
    if (await is_team_owner(ctx.message.mentions[0], ctx.message.role_mentions[0]) or
        ...
        await ctx.send(
            ":negative_squared_cross_mark: The mentioned user is currently a staff member. Demote them before releasing them.")
        return

    await rm_role(ctx.message.mentions[0], ctx.message.role_mentions[0])
    await set_free(ctx.message.mentions[0], ctx.message.guild)
    await ctx.send(
        ":exclamation: " + ctx.message.mentions[0].mention + " was released from " + ctx.message.role_mentions[
            0].mention)
...
@client.command(name='msgrole', aliases=['mr'], brief='Staff command. Message role.',
                description='Staff only. Message all members of a tagged role.', usage='[@tag_role] [string]')
...
@client.command(name='source', brief='Source code',
                description='Get information about the bot, as well as a link to the source code.', usage='')
